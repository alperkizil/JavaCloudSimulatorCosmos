...
/**
 * Verification test for MOEA Framework SPEA2 integration.
 *
 * Test scenario:
 ...
 * - MOEA Framework SPEA2
 * - MOEA Framework NSGA-II
 * - Native NSGA-II
 */
public class MOEA_SPEA2VerificationTest {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("╔══════════════════════════════════════════════════════════════════════╗");
        System.out.println("║         MOEA Framework SPEA2 Verification Test                       ║");
        ...
        // Configure optimization (shared configuration)
        ...
        // ════════════════════════════════════════════════════════════════
        // Run MOEA Framework SPEA2
        // ════════════════════════════════════════════════════════════════
        System.out.println("════════════════════════════════════════════════════════════════════════");
        System.out.println("MOEA FRAMEWORK SPEA2");
        System.out.println("════════════════════════════════════════════════════════════════════════");
        System.out.println();

        // Reset random state before SPEA2 run
        RandomGenerator.getInstance().reset(SEED);

        long spea2StartTime = System.currentTimeMillis();

        MOEA_SPEA2TaskSchedulingStrategy spea2Strategy = new MOEA_SPEA2TaskSchedulingStrategy(config);
        ParetoFront spea2Front = spea2Strategy.optimize(tasks, vms);

        long spea2Elapsed = System.currentTimeMillis() - spea2StartTime;
        System.out.println();
        System.out.println("SPEA2 completed in " + spea2Elapsed + " ms");
        System.out.println("Function evaluations: " + spea2Strategy.getLastEvaluationCount());
        System.out.println("Pareto front size: " + spea2Front.size());
        System.out.println();

        // ════════════════════════════════════════════════════════════════
        // Run MOEA Framework NSGA-II for comparison
        // ════════════════════════════════════════════════════════════════
        System.out.println("════════════════════════════════════════════════════════════════════════");
        System.out.println("MOEA FRAMEWORK NSGA-II (for comparison)");
        System.out.println("════════════════════════════════════════════════════════════════════════");
        System.out.println();

        // Reset random state before NSGA-II run
        RandomGenerator.getInstance().reset(SEED);

        long nsgaStartTime = System.currentTimeMillis();

        MOEA_NSGA2TaskSchedulingStrategy nsgaStrategy = new MOEA_NSGA2TaskSchedulingStrategy(config);
        ParetoFront nsgaFront = nsgaStrategy.optimize(tasks, vms);

        long nsgaElapsed = System.currentTimeMillis() - nsgaStartTime;
        System.out.println();
        System.out.println("NSGA-II completed in " + nsgaElapsed + " ms");
        System.out.println("Function evaluations: " + nsgaStrategy.getLastEvaluationCount());
        System.out.println("Pareto front size: " + nsgaFront.size());
        System.out.println();
        ...
        // SPEA2 Results
        System.out.println("SPEA2 Pareto Front:");
        printParetoFrontSummary(spea2Front);

        System.out.println();

        // MOEA NSGA-II Results
        System.out.println("MOEA NSGA-II Pareto Front:");
        printParetoFrontSummary(nsgaFront);
        ...
        // ════════════════════════════════════════════════════════════════
        // Verify SPEA2 solutions
        // ════════════════════════════════════════════════════════════════
        System.out.println();
        System.out.println("════════════════════════════════════════════════════════════════════════");
        System.out.println("SPEA2 SOLUTION VERIFICATION");
        System.out.println("════════════════════════════════════════════════════════════════════════");
        System.out.println();
        ...
        int verifyCount = Math.min(5, spea2Front.size()); // Verify first 5 solutions

        for (int i = 0; i < verifyCount; i++) {
            SchedulingSolution solution = spea2Front.getSolutions().get(i);

            // Get SPEA2 reported values
        ...
        System.out.println("════════════════════════════════════════════════════════════════════════");
        System.out.println();
        System.out.printf("%-30s %15s %15s %15s%n", "Metric", "SPEA2", "MOEA NSGA-II", "Native");
        System.out.printf("%-30s %15d %15d %15d%n", "Pareto front size:", spea2Front.size(), nsgaFront.size(), nativeFront.size());
        System.out.printf("%-30s %13d ms %13d ms %13d ms%n", "Execution time:", spea2Elapsed, nsgaElapsed, nativeElapsed);
        System.out.println();

        // Best solutions comparison
        SchedulingSolution spea2BestMakespan = spea2Front.getBestForObjective(0);
        SchedulingSolution spea2BestEnergy = spea2Front.getBestForObjective(1);
        SchedulingSolution spea2Knee = spea2Front.getKneePoint();

        SchedulingSolution nsgaBestMakespan = nsgaFront.getBestForObjective(0);
        SchedulingSolution nsgaBestEnergy = nsgaFront.getBestForObjective(1);
        SchedulingSolution nsgaKnee = nsgaFront.getKneePoint();
        ...
        System.out.printf("  SPEA2:       %.4f%n", spea2BestMakespan.getObjectiveValue(0));
        System.out.printf("  MOEA NSGA-II:%.4f%n", nsgaBestMakespan.getObjectiveValue(0));
        System.out.printf("  Native:      %.4f%n", nativeBestMakespan.getObjectiveValue(0));
        System.out.println();

        System.out.println("Best Energy (kWh):");
        System.out.printf("  SPEA2:       %.6f%n", spea2BestEnergy.getObjectiveValue(1));
        System.out.printf("  MOEA NSGA-II:%.6f%n", nsgaBestEnergy.getObjectiveValue(1));
        System.out.printf("  Native:      %.6f%n", nativeBestEnergy.getObjectiveValue(1));
        System.out.println();

        System.out.println("Knee Point (Balanced Trade-off):");
        System.out.printf("  SPEA2:       Makespan=%.4fs, Energy=%.6f kWh%n",
            spea2Knee.getObjectiveValue(0), spea2Knee.getObjectiveValue(1));
        System.out.printf("  MOEA NSGA-II:Makespan=%.4fs, Energy=%.6f kWh%n",
            nsgaKnee.getObjectiveValue(0), nsgaKnee.getObjectiveValue(1));
        System.out.printf("  Native:      Makespan=%.4fs, Energy=%.6f kWh%n",
            nativeKnee.getObjectiveValue(0), nativeKnee.getObjectiveValue(1));
        System.out.println();

        if (allPassed) {
            System.out.println("✓ SPEA2 INTEGRATION VERIFIED SUCCESSFULLY");
        } else {
        ...
        System.out.println("════════════════════════════════════════════════════════════════════════");
        System.out.println("SAMPLE SPEA2 KNEE POINT SOLUTION DETAILS");
        System.out.println("════════════════════════════════════════════════════════════════════════");
        System.out.println();
        printSolutionDetails(spea2Knee, tasks, vms);
    }
