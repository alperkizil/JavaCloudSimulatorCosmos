...
import com.cloudsimulator.PlacementStrategy.task.metaheuristic.objectives.LoadBalanceObjective;
...
public class LoadBalanceExperimentRunner {
    ...
    private static final String[] ALGORITHM_LABELS = {
        "GA_LoadBalance", "GA_Energy", "SA_LoadBalance", "SA_Energy",
        "NSGA-II", "SPEA-II"
    };
    ...
    public static void main(String[] args) {
        ...
        System.out.println("============================================================");
        System.out.println("  LOAD BALANCE vs ENERGY EXPERIMENT RUNNER");
        System.out.println("  Algorithms: GA(LB), GA(E), SA(LB), SA(E), NSGA-II, SPEA-II");
        System.out.println("  Objectives: LoadBalance (CV) vs Energy (kWh)");
    ...
    private static TaskAssignmentStrategy createStrategy(String label, List<Host> hosts, long seed) {
        switch (label) {
            case "GA_LoadBalance":
                return createGAStrategy(new LoadBalanceObjective(), createEnergyObjective(hosts));
            case "GA_Energy":
                return createGAStrategy(createEnergyObjective(hosts), new LoadBalanceObjective());
            case "SA_LoadBalance":
                return createSAStrategy(new LoadBalanceObjective(), createEnergyObjective(hosts));
            case "SA_Energy":
                return createSAStrategy(createEnergyObjective(hosts), new LoadBalanceObjective());
            case "NSGA-II":
    ...
    private static TaskAssignmentStrategy createNSGA2Strategy(List<Host> hosts, long seed) {
        LoadBalanceObjective loadBalance = new LoadBalanceObjective();
        EnergyObjective energy = new EnergyObjective();
        energy.setHosts(hosts);
    ...
    private static AlgorithmResult runAlgorithm(String label,
        ...
        double actualLoadBalance = computeSimulatedLoadBalance(context);
        double actualEnergyKWh = energyCalcStep.getTotalITEnergyKWh();
        double actualMakespan = taskExecStep.getMakespan();
        System.out.printf("  [DEBUG] Selected solution simulation: loadBalance=%.6f, energy=%.6f kWh, makespan=%d s%n",
            actualLoadBalance, actualEnergyKWh, (long) actualMakespan);
        ...
        if (strategy instanceof MultiObjectiveTaskSchedulingStrategy) {
            ...
            if (solutions.isEmpty()) {
                ...
                solutions.add(new double[]{actualLoadBalance, actualEnergyKWh});
            }
        } else {
            solutions = new ArrayList<>();
            solutions.add(new double[]{actualLoadBalance, actualEnergyKWh});
    ...
    private static List<double[]> simulateAllParetoSolutions(
        ...
        for (int i = 0; i < paretoSolutions.size(); i++) {
            ...
            double simLoadBalance = computeSimulatedLoadBalance(context);
            double simEnergy = energyCalc.getTotalITEnergyKWh();
            simulatedResults.add(new double[]{simLoadBalance, simEnergy});

            if ((i + 1) % 10 == 0 || i == paretoSolutions.size() - 1) {
                System.out.printf("    Simulated %d/%d solutions (latest: loadBalance=%.6f, energy=%.6f kWh)%n",
                    i + 1, paretoSolutions.size(), simLoadBalance, simEnergy);
            }
        }
    ...
    // =========================================================================
    // SIMULATED LOAD BALANCE COMPUTATION
    // =========================================================================

    /**
     * Computes load balance (CV of VM completion times) from simulation state.
     * Matches the analytical LoadBalanceObjective calculation.
     */
    private static double computeSimulatedLoadBalance(SimulationContext context) {
        List<VM> vms = context.getVms();
        if (vms == null || vms.isEmpty()) return 0.0;

        List<Double> completionTimes = new ArrayList<>();
        for (VM vm : vms) {
            if (!vm.isAssignedToHost()) continue;
            int taskCount = vm.getFinishedTasks().size() + vm.getAssignedTasks().size();
            if (taskCount == 0) continue;
            long execSeconds = vm.getTotalActiveWorkloadSeconds();
            completionTimes.add((double) execSeconds);
        }

        if (completionTimes.size() <= 1) return 0.0;

        double mean = completionTimes.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);
        if (mean == 0.0) return 0.0;

        double sumSquaredDiffs = completionTimes.stream()
            .mapToDouble(ct -> (ct - mean) * (ct - mean))
            .sum();
        double stdDev = Math.sqrt(sumSquaredDiffs / completionTimes.size());
        return stdDev / mean;
    }
    ...
    private static void generateScenarioCSVs(ScenarioResult result) {
        ...
        try (PrintWriter w = new PrintWriter(new FileWriter(outputDir.resolve(graphFile).toFile()))) {
            w.println("Algorithm,Seed,LoadBalance,Energy,IsUniversalPareto");
    ...
    private static void generateExperimentSummary(List<ScenarioResult> allResults) {
        ...
        try (PrintWriter w = new PrintWriter(new FileWriter(filePath.toFile()))) {
            w.println("Scenario,ScenarioName,Algorithm,Seed,HV,GD,IGD,Spacing,NonDomSolutions,ParetoContribution,TimeMs,LoadBalance_Best,Energy_Best");
