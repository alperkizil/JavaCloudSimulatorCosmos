...
public class PowerCeilingWaitingTimeExperimentRunner {
    ...
    // Backward-compatible parity hooks: defaults equal the production values, so
    // normal runs are unaffected; the reduced-scale byte-diff gate overrides them.
    ...
    private static final String REPORTS_BASE_DIR = "reports/powerceiling";
    ...
    static class AlgorithmResult {
        ...
        // Parallel to solutions. One peak-aggregate-DC-power value (Watts) per
        // scheduled solution, computed post-hoc via PowerCeilingEnergyObjective.
        // Feasibility at each cap level is derived: peakPower <= cap.
        List<Double> peakPowersWatts;
    ...
    public static void main(String[] args) {
        ...
        int scenarioLimit = Math.min(3, Integer.getInteger("parity.scenarios", 3));
            ...
            for (String label : activeLabels()) {
                ...
                for (int run = 0; run < NUM_RUNS; run++) {
                    ...
                    try {
                        ...
                        AlgorithmResult failed = new AlgorithmResult(label, seed, fallback, 0);
                        failed.peakPowersWatts = new ArrayList<>();
                        failed.peakPowersWatts.add(0.0);
                        seedResults.add(failed);
                    }
                }
        ...
        // Try to run Python plotter (skippable for the parity gate)
    ...
    /**
     * The canonical {@link #ALGORITHM_LABELS}, optionally filtered by the
     * comma-separated {@code -Dparity.algos} property (order preserved). Returns the
     * full set when the property is absent, so production runs are unaffected.
     */
    ...
    private static AlgorithmResult runAlgorithm(String label,
        ...
        // Safety: align lengths (should already match by construction)
        while (peakPowers.size() < solutions.size()) peakPowers.add(0.0);

        long endTime = System.currentTimeMillis();
        AlgorithmResult result = new AlgorithmResult(label, seed, solutions, endTime - startTime);
        result.peakPowersWatts = peakPowers;
        return result;
    }
    ...
    // =========================================================================
    // POWER-CEILING MEASUREMENT (additive; no mutation of simulator state)
    // =========================================================================

    /**
     * Returns a peak-aggregate-DC-power value (Watts) for each SchedulingSolution
     * in the strategy's Pareto front, in the same order returned by
     * {@link ParetoFront#getSolutions()} and {@link #simulateAllParetoSolutions}.
     * Uses PowerCeilingEnergyObjective's sweep-line; does NOT re-run the simulator.
     */
    private static List<Double> measureParetoFrontPower(
            TaskAssignmentStrategy strategy, SimulationContext context) {

        List<Double> peaks = new ArrayList<>();
        if (!(strategy instanceof MultiObjectiveTaskSchedulingStrategy)) return peaks;

        ParetoFront front = ((MultiObjectiveTaskSchedulingStrategy) strategy).getLastParetoFront();
        if (front == null || front.isEmpty()) return peaks;
        ...
        PowerCeilingEnergyObjective meter = new PowerCeilingEnergyObjective();
        meter.setHosts(hosts);

        for (SchedulingSolution sol : front.getSolutions()) {
            meter.evaluate(sol, context.getTasks(), runningVMs);
            peaks.add(meter.getLastPeakPower());
        }
        return peaks;
    }

    /**
     * Builds a SchedulingSolution from the current task->VM assignments on the
     * context (populated by baselines and single-objective metaheuristics), then
     * measures peak aggregate DC power via PowerCeilingEnergyObjective.
     */
    private static double measureCurrentAssignmentPower(SimulationContext context) {
        List<VM> runningVMs = new ArrayList<>();
        for (VM vm : context.getVms()) {
            if (vm.isAssignedToHost()) runningVMs.add(vm);
        }
        if (runningVMs.isEmpty()) return 0.0;
        ...
        SchedulingSolution sol = new SchedulingSolution(tasks.size(), runningVMs.size(), 2);
        for (int t = 0; t < tasks.size(); t++) {
            Long assignedVmId = tasks.get(t).getAssignedVmId();
            if (assignedVmId != null && vmIdToIndex.containsKey(assignedVmId)) {
                sol.setAssignedVM(t, vmIdToIndex.get(assignedVmId));
            } else {
                sol.setAssignedVM(t, 0); // unassigned → VM 0 (will show as zero inc power)
            }
        }
        sol.rebuildTaskOrdering();

        PowerCeilingEnergyObjective meter = new PowerCeilingEnergyObjective();
        meter.setHosts(context.getHosts());
        meter.evaluate(sol, tasks, runningVMs);
        return meter.getLastPeakPower();
    }
    ...
    private static void generateScenarioCSVs(ScenarioResult result) {
        ...
        try (PrintWriter w = new PrintWriter(new FileWriter(outputDir.resolve(graphFile).toFile()))) {
            w.println("Algorithm,Seed,WaitingTime,Energy,PeakPowerWatts,IsUniversalPareto");

            for (Map.Entry<String, List<AlgorithmResult>> entry : result.perSeedResults.entrySet()) {
                for (AlgorithmResult ar : entry.getValue()) {
                    for (int i = 0; i < ar.solutions.size(); i++) {
                        double[] sol = ar.solutions.get(i);
                        ...
                        w.printf("%s,%d,%.6f,%.9f,%.3f,%b%n",
                            ar.label, ar.seed, sol[0], sol[1], peak, isUniv);
                    }
                }
            }
    ...
    private static void generateExperimentSummary(List<ScenarioResult> allResults) {
        ...
        try (PrintWriter w = new PrintWriter(new FileWriter(filePath.toFile()))) {
            w.println("Scenario,ScenarioName,Algorithm,Seed,HV,GD,IGD,Spacing,NonDomSolutions,ParetoContribution,TimeMs,WaitingTime_Best,Energy_Best,PeakPower_Min,PeakPower_Mean");
            ...
            for (ScenarioResult sr : allResults) {
                for (Map.Entry<String, List<AlgorithmResult>> entry : sr.perSeedResults.entrySet()) {
                    ...
                    for (AlgorithmResult ar : seeds) {
                        ...
                        int ndCount = ar.nonDominatedSolutions != null
                            ? ar.nonDominatedSolutions.size() : ar.solutions.size();
                        double peakMin = (ar.peakPowersWatts == null || ar.peakPowersWatts.isEmpty())
                            ? 0.0 : ar.peakPowersWatts.stream().mapToDouble(Double::doubleValue).min().orElse(0.0);
                        double peakMean = (ar.peakPowersWatts == null || ar.peakPowersWatts.isEmpty())
                            ? 0.0 : ar.peakPowersWatts.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);

                        w.printf("%d,%s,%s,%d,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%.6f,%.9f,%.3f,%.3f%n",
                            ...
                            bestWaitingTime, bestEnergy, peakMin, peakMean);
                    }
                ...
                long avgTime = successfulRuns > 0
                    ...
                    double peakMinAll = seeds.stream()
                        .filter(a -> a.peakPowersWatts != null)
                        .flatMap(a -> a.peakPowersWatts.stream())
                        .mapToDouble(Double::doubleValue).min().orElse(0.0);
                    double peakMeanAll = seeds.stream()
                        .filter(a -> a.peakPowersWatts != null)
                        .flatMap(a -> a.peakPowersWatts.stream())
                        .mapToDouble(Double::doubleValue).average().orElse(0.0);

                    w.printf("%d,%s,%s,MEAN,%.6f,%.6f,%.6f,%.6f,,%d,%d,%.6f,%.9f,%.3f,%.3f%n",
                        sr.scenarioNumber, sr.scenarioName, label,
                        mean(hvs), mean(gds), mean(igds), mean(spacings),
                        unionPCont, avgTime,
                        bestWaitingTimeAll, bestEnergyAll,
                        peakMinAll, peakMeanAll);
                }
            }
    ...
    // =========================================================================
    // CSV OUTPUT — POWER-CEILING FEASIBILITY (one row per (scenario, algo, seed, cap))
    // =========================================================================

    private static void generatePowerCeilingFeasibilityCSV(List<ScenarioResult> allResults) {
        Path filePath = Paths.get(REPORTS_DIR, "power_ceiling_feasibility.csv");

        try (PrintWriter w = new PrintWriter(new FileWriter(filePath.toFile()))) {
            w.println("Scenario,ScenarioName,Algorithm,Seed,CapWatts,TotalSolutions,FeasibleSolutions,FeasibilityRate,PeakPowerMax");
            ...
            for (ScenarioResult sr : allResults) {
                for (Map.Entry<String, List<AlgorithmResult>> entry : sr.perSeedResults.entrySet()) {
                    ...
                    for (AlgorithmResult ar : entry.getValue()) {
                        ...
                        double peakMax = ar.peakPowersWatts.stream()
                            .mapToDouble(Double::doubleValue).max().orElse(0.0);
                        for (double cap : POWER_CAP_LEVELS_WATTS) {
                            int total = ar.peakPowersWatts.size();
                            long feasible = ar.peakPowersWatts.stream()
                                .filter(p -> p <= cap).count();
                            double rate = total > 0 ? (double) feasible / total : 0.0;
                            w.printf("%d,%s,%s,%d,%.1f,%d,%d,%.4f,%.3f%n",
                                sr.scenarioNumber, sr.scenarioName, label, ar.seed,
                                cap, total, feasible, rate, peakMax);
                        }
                    }
                }
            }

            System.out.println("  Wrote: power_ceiling_feasibility.csv");
        } catch (IOException e) {
            System.err.println("  ERROR writing power_ceiling_feasibility.csv: " + e.getMessage());
        }
    }
