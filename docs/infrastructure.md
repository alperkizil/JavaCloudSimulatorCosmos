...
define (identically for all three studies):

- **1 datacenter**: `DC-Experiment`, capacity 50 hosts, power budget 400,000 W.
- **40 hosts** in three groups, all on `MeasurementBasedPowerModel`
  (`toExperimentConfiguration` *throws* if any host group uses another model,
  unless `allowNonMeasurementPowerModel` is explicitly set):
  - 16 × CPU_ONLY — 2.5 G IPS/core, 16 cores, 0 GPUs
  - 12 × GPU_ONLY — 2.8 G IPS/core, 8 cores, 4 GPUs
  - 12 × CPU_GPU_MIXED — 3.0 G IPS/core, 32 cores, 4 GPUs
- **60 VMs** owned by the single user `ExperimentUser`: 20 CPU / 20 GPU /
  20 MIXED, each split 8 "fast" (5 G IPS/vCPU) + 8 "medium" (2 G) +
  4 "slow" (0.5 G), all with 4 vCPUs.
- **Tasks** generated by `generateTasks(...)`: pure and RNG-free, cycling
  through the workload types and the 16 log-spaced `instructionLengths`
  (0.5 G … 25.16 G instructions, the "LOG16" workload) with decoupled phases
  so every (type, length) pair occurs. Scenario task counts (CPU/GPU):
  Balanced 250/250, GPU_Stress 100/400, CPU_Stress 400/100.
