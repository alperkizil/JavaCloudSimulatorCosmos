...
makespan) but costs far more energy.

Source: `MeasurementBasedPowerModel.java:529-549`.

## 1b.7 Hardware scale factor

The `hardwareScaleFactor` lets the model represent hosts that are
stronger or weaker than the Dell Precision 7920 reference system.
...
In the current experiment all hosts use scale factor 1.0.

Source: `PowerModelFactory.java:80-93`.

## 1b.8 Factory bridge: backward compatibility

The rest of the simulator expects the legacy `PowerModel` interface
(five-parameter: maxCpu, maxGpu, idleCpu, idleGpu, other). When a host
is configured with `"MeasurementBasedPowerModel"`, the
`PowerModelFactory` creates a `PowerModel` wrapper whose parameters are
derived from the measurement data:

| Parameter      | Value                         | Origin                               |
|----------------|-------------------------------|--------------------------------------|
| Max CPU power  | 158.76 W                      | 25 W idle + 133.76 W (Cinebench)     |
| Max GPU power  | 367.18 W                      | 15 W idle + 352.18 W (FurMark)       |
| Idle CPU power | 25.0 W                        | ~33 % of 75.79 W system idle         |
| Idle GPU power | 15.0 W                        | ~20 % of 75.79 W system idle         |
| Other          | 35.79 W                       | ~47 % of 75.79 W system idle         |

This wrapper ensures that any code path still calling the old
`calculateTotalPower(cpuUtil, gpuUtil)` method gets numbers grounded in
real measurements, while the energy objective itself calls the richer
workload-aware methods directly.
