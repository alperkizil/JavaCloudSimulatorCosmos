...
while evaluations < 40,000 + 10,200:
    scale <- T / T0                                               # mutation step size   <-- CHANGED (PR #250)
    repeat 200 times:                                             # iterations per temperature
        s' <- s with each task gene mutated at prob 0.05 × scale; if that expects ≤ 1 move, exactly 1 move; repair; evaluate   <-- CHANGED (PR #250)
